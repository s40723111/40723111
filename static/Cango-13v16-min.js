...
var T
T=o===l?-1:1
var S=r*r,k=a*a,_=S*k-S*b*b-k*w*w,P=S*b*b+k*w*w,A=T*Math.sqrt(Math.abs(_/P))
u=A*r*b/a+(t+s)/2,f=A*-a*w/r+(e+i)/2,d=Math.asin(parseFloat(((e-f)/a).toFixed(9))),p=Math.asin(parseFloat(((i-f)/a).toFixed(9))),u>t&&(d=Math.PI-d),u>s&&(p=Math.PI-p),0>d&&(d=2*Math.PI+d),0>p&&(p=2*Math.PI+p),l&&d>p&&(d-=2*Math.PI),!l&&p>d&&(p-=2*Math.PI)}var I=p-d
...
t.push({type:0===e?"M":"L",values:[s.x,s.y]})}return t.push({type:"Z",values:[]}),t}}()
var w=function(){return m+=1}
return void 0===shapeDefs&&(shapeDefs={circle:function(t){var e=t||1
...
if(!(Math.abs(r-a.rawWidth)/r<.01&&Math.abs(n-a.rawHeight)/n<.01)&&(a.bkgCanvas.timeline&&a.bkgCanvas.timeline.animTasks.length&&a.deleteAllAnimations(),a.rawWidth=r,a.rawHeight=n,a.aRatio=r/n,a.bkgCanvas===a.cnvs))for(a.cnvs.width=r,a.cnvs.height=n,t=1;t<a.bkgCanvas.layers.length;t++)e=a.bkgCanvas.layers[t].cElem,e&&(e.style.top=s+"px",e.style.left=i+"px",e.style.width=r+"px",e.style.height=n+"px",e.width=r,e.height=n)}var i,r,a=this
if("string"==typeof t&&document.getElementById(t)){if(this.cnvs=document.getElementById(t),this.cId=t,!(this.cnvs instanceof HTMLCanvasElement))return void alert("element not a canvas");-1!==this.cId.indexOf("_ovl_")?(this.cgoType="OVL",i=this.cId.slice(0,this.cId.indexOf("_ovl_")),this.bkgCanvas=document.getElementById(i)):(this.cgoType="BKG",this.bkgCanvas=this.cnvs),this.rawWidth=this.cnvs.offsetWidth,this.rawHeight=this.cnvs.offsetHeight}else{if(!(t instanceof HTMLCanvasElement))return void alert("element not a canvas")
this.cnvs=t,this.bkgCanvas=this.cnvs,this.rawWidth=this.cnvs.width,this.rawHeight=this.cnvs.height,document.contains(t)?(this.cId=this.cnvs.id,this.cgoType="BKG",this.cId||(this.cId="_bkg_"+w(),this.cnvs.id=this.cId)):(this.cId="_os_"+w(),this.cgoType="OS")}this.aRatio=this.rawWidth/this.rawHeight,this.widthPW=100,this.heightPW=100/this.aRatio,this.bkgCanvas.hasOwnProperty("layers")||(this.bkgCanvas.layers=[],r=new f(this.cId,this.cnvs),this.bkgCanvas.layers[0]=r,"OS"!==this.cgoType&&e(s,250)),void 0===u||this.bkgCanvas.hasOwnProperty("timeline")||(this.bkgCanvas.timeline=new u),this.cnvs.hasOwnProperty("resized")||(this.cnvs.width=this.rawWidth,this.cnvs.height=this.rawHeight,this.cnvs.resized=!0),this.ctx=this.cnvs.getContext("2d"),this.yDown=!0,this.vpW=this.rawWidth,this.vpH=this.rawHeight,this.vpOrgX=0,this.vpOrgYsvg=0,this.vpOrgYrhc=this.rawHeight,this.vpOrgY=this.vpOrgYsvg,this.xscl=1,this.yscl=1,this.xoffset=0,this.yoffset=0,this.savWC={xscl:this.xscl,yscl:this.yscl,xoffset:this.xoffset,yoffset:this.yoffset},this.ctx.textAlign="left",this.ctx.textBaseline="alphabetic",this.penCol="rgba(0, 0, 0, 1.0)",this.penWid=1,this.lineCap="butt",this.paintCol="rgba(128,128,128,1.0)",this.fontSize=12,this.fontWeight=400,this.fontFamily="Consolas, Monaco, 'Andale Mono', monospace",this.clipCount=0,v(this)},Cango.prototype.animation=function(t,e,s,i){var r,a,n=w()
return a=this.cId+"_"+n,r=new d(a,this,t,e,s,i),this.stopAnimation(),this.bkgCanvas.timeline.animTasks.push(r),r.id},Cango.prototype.pauseAnimation=function(){this.bkgCanvas.timeline.pauseAnimation()},Cango.prototype.playAnimation=function(t,e){this.bkgCanvas.timeline.playAnimation(t,e)},Cango.prototype.stopAnimation=function(){this.bkgCanvas.timeline.stopAnimation()},Cango.prototype.stepAnimation=function(){this.bkgCanvas.timeline.stepAnimation()},Cango.prototype.deleteAnimation=function(t){var e=this
...
n&&o.transform.rotate(n),1!==a&&o.transform.scale(a),(i||r)&&o.transform.translate(i,r),o.fillCol="rgba(0,0,0,1.0)",o.clearFlag=!0,this.render(o)},Cango.prototype.createLayer=function(){var t,e,s,i,r,a=this.rawWidth,n=this.rawHeight,o=w(),h=this.bkgCanvas.layers.length
return"OVL"===this.cgoType||"OS"===this.cgoType?(console.log("offscreen canvases and layers cannot create layers"),""):(s=this.cId+"_ovl_"+o,t="<canvas id='"+s+"' style='position:absolute' width='"+a+"' height='"+n+"'></canvas>",r=this.bkgCanvas.layers[h-1].cElem,r.insertAdjacentHTML("afterend",t),e=document.getElementById(s),e.style.backgroundColor="transparent",e.style.left=this.bkgCanvas.offsetLeft+this.bkgCanvas.clientLeft+"px",e.style.top=this.bkgCanvas.offsetTop+this.bkgCanvas.clientTop+"px",e.style.width=this.bkgCanvas.offsetWidth+"px",e.style.height=this.bkgCanvas.offsetHeight+"px",i=new f(s,e),this.bkgCanvas.layers.push(i),s)},Cango.prototype.deleteLayer=function(t){var e,s
